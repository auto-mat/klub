...
from .serializers import (
    ...
    InteractionSerizer,
    MoneyAccountCheckSerializer,
    PaymentSerializer,
    ProfileSerializer,
    ResetPasswordbyEmailConfirmSerializer,
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            contact, created = CompanyContact.objects.get_or_create(
                ...
                contact_first_name=serializer.validated_data.get(
                    "contact_first_name", ""
                ),
                contact_last_name=serializer.validated_data.get(
                    "contact_last_name", ""
                ),
            )
...
class CheckPaymentView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            payments = dpch.payment_set.order_by("date").filter(
                amount=serializer.validated_data["amount"],
                type="bank-transfer",
                date__gte=serializer.validated_data["date"],
                date__lte=serializer.validated_data["date"]
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            int_type, created = InteractionType.objects.get_or_create(
                ...
                category=category,
                defaults={
                    "name": serializer.validated_data["interaction_type"],
                    "summary_bool": True,
                },
            )
            ...
            Interaction.objects.create(
                ...
                event=serializer.validated_data["event"],
                administrative_unit=serializer.validated_data[
                    "event"
                ].administrative_units.first(),
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            if serializer.validated_data.pop("profile_type") == "user":
                email = ProfileEmail.objects.filter(
                    email=serializer.validated_data.pop("email")
                )
                profile = "user"
            else:
                email = CompanyContact.objects.filter(
                    email=serializer.validated_data.pop("email")
                )
                profile = "company"
            if email.exists():
                ...
                if user_channels.exists():
                    ...
                    payment.save()
                    return Response(
                        ProfileSerializer(user_channel.user).data,
...
class ResetPasswordbyEmailView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if emails.exists():
            ...
            administrative_unit = AdministrativeUnit.objects.filter(
                from_email_str__isnull=False
            ).first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            user = UserProfile.objects.get(
                id=urlsafe_base64_decode(kwargs["uid"]).decode()
            )
...
class EventViewMixin:
    ...
    def get_queryset(self):
        return (
            Event.objects.filter(public_on_web=True)
