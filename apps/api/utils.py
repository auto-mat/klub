    ...
    if not found_payment:
        ...
        for dpch in user.userchannels.all():
            ...
            if isinstance(api, ApiAccount) and dpch.payment_total == 0:
                ...
                url = "https://www.darujme.cz/api/v1/organization/{0}/pledges-by-filter/" \
                        "?apiId={1}&apiSecret={2}&projectId={3}&fromPledgedDate={4}"\
                        .format(
                ...
                for pledge in response.json()['pledges']:
                    if pledge['donor']['email'] == user.get_email_str():
                        ...
                        for transaction in transactions:
                            ...
                            if transaction['state'] in ['success', 'sent_to_organization', 'success_money_on_account']:
                                # we are checking if the user just "donated", we dont know how many... but we let him in.
                                found_payment = True
                                break
        if found_payment:
            cache.set(f"{user.id}_paid_section", True, timeout=60*60*3)  # set cache for 3 hours
