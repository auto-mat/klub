...
class Query(__Query__):
    def get_timeline_context(self):  # noqa
        ...
        profiles = Profile.objects.filter(
            pk__in=self.params.get("search_profile_pks", [])
        )
        for profile in profiles:
            if self.huser.user.can_administer_profile(profile):
                ...
                for interaction in profile.interaction_set.select_related(
                    "type__category"
                ).all():
                        ...
                        events.append(
                            {
                                ...
                                "start_date": self.mk_timeline_date(
                                    localtime(interaction.date_from)
                                ),
                                "end_date": self.mk_timeline_date(
                                    localtime(is_period(interaction))
                                ),
                                "text": {
                    ...
                    events.append(
                        {
                            ...
                            "start_date": self.mk_timeline_date(
                                datetime.datetime.combine(
                                    payment.date, datetime.time(0, 0)
                                ),
                            ),
                            "text": {
                                "headline": get_unit_color(
