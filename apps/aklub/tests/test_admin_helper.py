...
class TestProfilePostMixin:
    ...
    def get_profile_post_data(self, administrative_units, event, index, bank_account, test_str, action, child_model):
        if action == 'add':
            profile_post_data = {
    ...
    def compare_profile_personal_info(self, action, post_data, profile):
        ...
        if profile.is_userprofile():
            email = set(ProfileEmail.objects.filter(user=profile).values_list('email', flat=True),)
        else:
            email = set(CompanyContact.objects.filter(company=profile).values_list('email', flat=True))
