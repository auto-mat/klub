...
class TimelineTest(CreateSuperUserMixin, TestCase):
    ...
    def test_timeline_userprofile(self):
        foo_user = user_profile_recipe.make(
            id=2978, first_name="Foo", email="foo@bar.cz"
        )
        unit = mommy.make("aklub.AdministrativeUnit")
        bank_acc = mommy.make("aklub.BankAccount", administrative_unit=unit)
        foo_user_pc = donor_payment_channel_recipe.make(
            user=foo_user, money_account=bank_acc
        )
        mommy.make(
            "aklub.Payment",
            amount=350,
        ...
        int_type = mommy.make(
            "interactions.InteractionType", name="testtype", category=category
        )
        mommy.make(
    ...
    def test_timeline_companyprofile(self):
        company = mommy.make(
            "aklub.companyprofile", id=999, name="company_name", email="foo@bar.cz"
        )
        unit = mommy.make("aklub.AdministrativeUnit")
        ...
        int_type = mommy.make(
            "interactions.InteractionType", name="test_company_type", category=category
        )
        mommy.make(
            "interactions.Interaction",
            type=int_type,
