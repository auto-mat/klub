...
class PaymentsImportExportTests(CreateSuperUserMixin, TransactionTestCase):
    ...
    def test_payments_import(self):
        ...
        csv_file_create_interactions = os.path.join(
            p.parents[1], "test_data", "create_payments.csv"
        )
        ...
        file_name = [
            val.split("=")[-1].replace('"', "")
            for val in result.group(0).split(" ")
            if "value" in val
        ][0]
        post_data = {
        ...
        self.assertRedirects(
            response, expected_url=reverse("admin:aklub_payment_changelist")
        )
...
class InteractionsImportExportTests(CreateSuperUserMixin, TransactionTestCase):
    ...
    def test_interaction_import(self):
        ...
        p = pathlib.PurePath(__file__)
        csv_file_create_interactions = os.path.join(
            p.parents[1], "test_data", "create_interactions.csv"
        )
        ...
        response = self.client.post(address, post_data)
        self.assertRedirects(
            response, expected_url=reverse("admin:interactions_interaction_changelist")
        )
        ...
        self.assertEqual(
            int1.next_communication_date.isoformat(), "2020-02-19T12:08:42+00:00"
        )
        ...
        self.assertEqual(
            int3.user.get_email_str(int3.administrative_unit), "test_email@email.com"
        )
...
class DonorImportExportTests(CreateSuperUserMixin, TransactionTestCase):
    ...
    def test_dpch_import(self):
        ...
        p = pathlib.PurePath(__file__)
        csv_file_create_dpch = os.path.join(
            p.parents[1], "test_data", "test_donor_import.csv"
        )
        ...
        response = self.client.post(address, post_data)
        self.assertRedirects(
            response, expected_url=reverse("admin:aklub_donorpaymentchannel_changelist")
        )
        ...
        self.assertEqual(
            new_dpch.expected_date_of_first_payment, datetime.date(2016, 9, 16)
        )
        ...
        # CompanyProfile
        new_dpch2 = DonorPaymentChannel.objects.get(
            user=self.company, event=self.event1
        )
        ...
        self.assertEqual(
            new_dpch2.expected_date_of_first_payment, datetime.date(2016, 2, 12)
        )
        ...
        self.assertEqual(
            dpch_update.expected_date_of_first_payment, datetime.date(2016, 1, 11)
        )
        ...
        self.assertEqual(
            dpch_update.end_of_regular_payments, datetime.date(2017, 1, 11)
        )
...
class AdminImportExportTests(CreateSuperUserMixin, TransactionTestCase):
    ...
    def create_profiles(self):
        ...
        for index, profile_type in enumerate(profiles_data):
            ...
            mommy.make(
                "aklub.DonorPaymentChannel",
                user=user,
                expected_date_of_first_payment=datetime.datetime.strptime(
                    "2015-12-16", "%Y-%m-%d"
                ),
                no_upgrade=False,
    ...
    def test_userprofile_import(self):
        ...
        response = self.client.post(address, post_data)
        self.assertRedirects(
            response, expected_url=reverse("admin:aklub_userprofile_changelist")
        )
        ...
        user_bank_account = UserBankAccount.objects.filter(
            bank_account_number="9988776655/0100"
        )
        self.assertEqual(user_bank_account.count(), 1)
        self.assertEqual(donor[0].user_bank_account, user_bank_account[0])
        self.assertEqual(
            user_profile[0].polymorphic_ctype,
            ContentType.objects.get(model="userprofile"),
        )
        ...
        self.assertEqual(
            user_profile[0].administrative_units.all().values_list("name")[0], ("AU2",)
        )
        ...
        self.assertEqual(donor[0].money_account, bank_account[0])
        user_bank_account = UserBankAccount.objects.filter(
            bank_account_number="1111111111/0100"
        )
        self.assertEqual(user_bank_account.count(), 1)
        self.assertEqual(
            user_profile[0].polymorphic_ctype,
        ...
        preference = Preference.objects.filter(
            user=user_profile[0], administrative_unit=1
        )
        self.assertEqual(preference[0].send_mailing_lists, False)
        self.assertEqual(preference[0].letter_on, False)
        self.assertEqual(
            set(user_profile[0].administrative_units.all().values_list("name")),
            {("AU1",), ("AU2",)},
        )
    ...
    def test_companyprofile_minimal_fields_import(self):
        ...
        company_profile_count = (
            CompanyProfile.objects.count() + number_of_new_company_profiles
        )
        response = self.client.post(address, post_data)
        self.assertRedirects(
            response, expected_url=reverse("admin:aklub_companyprofile_changelist")
        )
    ...
    def test_companyprofile_import(self):
        ...
        company_profile = Profile.objects.filter(
            email="test.companyprofile@companyprofile.test"
        )
        ...
        self.assertEqual(donor[0].money_account, bank_account[0])
        user_bank_account = UserBankAccount.objects.filter(
            bank_account_number="5554443331/0900"
        )
        self.assertEqual(user_bank_account.count(), 1)
        self.assertEqual(donor[0].user_bank_account, user_bank_account[0])
        self.assertEqual(
            company_profile[0].polymorphic_ctype,
            ContentType.objects.get(model="companyprofile"),
        )
        ...
        self.assertEqual(preference[0].letter_on, True)
        self.assertEqual(
            company_profile[0].administrative_units.all().values_list("name")[0],
            ("AU2",),
        )
        company_contact = company_profile[0].companycontact_set.first()
        self.assertEqual(
            company_contact.email, "test.companyprofile@companyprofile.test"
        )
        ...
        self.assertEqual(preference_count, 2)
        preference = Preference.objects.filter(
            user=company_profile[0], administrative_unit=1
        )
        self.assertEqual(preference[0].send_mailing_lists, False)
        self.assertEqual(preference[0].letter_on, False)
        self.assertEqual(
