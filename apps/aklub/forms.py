...
class TaxConfirmationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["profile"] = forms.ModelMultipleChoiceField(
            queryset=profiles, widget=FilteredSelectMultiple("UserProfile", False)
        )
...
class UserCreateForm(UserCreationForm):
    ...
    hidden_lock_change = forms.CharField(
        widget=forms.HiddenInput(), initial="locked", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            ...
            "You can set password in the next step or anytime in user detail form"
        )
...
class UnitUserProfileAddForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "administrative_units"
        ].queryset = self.request.user.administrated_units.all()
        self.fields["administrative_units"].required = True
...
class UnitUserProfileChangeForm(UnitUserProfileAddForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "administrative_units"
        ].queryset = self.instance.administrative_units.all()
...
class CompanyProfileAddForm(forms.ModelForm):
    ...
    def clean(self):
        if (
            self.cleaned_data.get("crn") is None
            and self.cleaned_data.get("no_crn_check") is False
        ):
    ...
    def save(self, commit=True):
        ...
        if (
            email
            or self.cleaned_data["telephone"]
            or self.cleaned_data["contact_first_name"]
            or self.cleaned_data["contact_last_name"]
        ):
