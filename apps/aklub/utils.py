...
def check_annotate_subqueries(self, request):
    ...
    next_com = Interaction.objects.filter(
        user=models.OuterRef("pk"), **unit_filter
    ).order_by("-next_communication_date")
    annotate_kwargs = {
        "next_communication_date": models.Subquery(
            next_com.values("next_communication_date")[:1],
