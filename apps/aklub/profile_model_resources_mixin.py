...
def get_preference_model_custom_field_dehydrate_func():
    ...
    for field in fields:
        ...
        funcs[func_name] = (preference_model_dehydrate_decorator(field=field))(
            dehydrate_base
        )
...
def import_obj(self, obj, data, dry_run):  # noqa
    ...
    if data.get("date_joined"):
        ...
        try:
            local_time = datetime.datetime.strptime(
                data.get("date_joined"), "%Y-%m-%d %H:%M:%S"
            )
    ...
    if obj.is_userprofile():
        ...
        if data.get("telephone") or data.get("email"):
            ...
            if (
                not company_contact.contact_first_name
                and not company_contact.contact_last_name
            ):
    ...
    if data.get("administrative_units"):
        ...
        check["preference"], _ = Preference.objects.get_or_create(
            user=obj,
            administrative_unit=obj.administrative_units.get(
                id=data["administrative_units"]
            ),
        )
    ...
    if data.get("event") and data.get("bank_account") and data.get("donor") == "x":
        ...
        if can_create_dpch:
            ...
            if data.get("user_bank_account"):
                check["user_bank_account"], _ = UserBankAccount.objects.get_or_create(
...
def dehydrate_donor(self, profile):
    ...
    for donor in profile.userchannels.all():
        if donor:
            ...
            try:
                donor_list.append(
                    f"bank_accout:{donor.money_account.bank_account_number}\n"
                )
            except AttributeError:
                donor_list.append("bank_accout:\n")
            try:
                donor_list.append(
                    f"user_bank_account:{donor.user_bank_account.bank_account_number}\n"
                )
            except AttributeError:
