...
class EventMixin(forms.ModelForm):
    Event = forms.ModelChoiceField(
        queryset=Event.objects.filter(
            slug__isnull=False, enable_registration=True
        ).exclude(slug=""),
        to_field_name="slug",
    )
...
class FieldNameMappingMixin(object):
    def add_prefix(self, field_name):
        field_name = self.FIELD_NAME_MAPPING.get(
            field_name, super().add_prefix(field_name)
        )
        return field_name


class RegularDarujmeUserForm_UserProfile(
    FieldNameMappingMixin, RegularUserForm_UserProfile
):
    FIELD_NAME_MAPPING = {
...
class RegularDarujmeUserForm_DonorPaymentChannel(
    FieldNameMappingMixin, RegularUserForm_DonorPaymentChannel
):
...
class RegularUserForm_DonorPaymentChannelDPNK(RegularUserForm_DonorPaymentChannel):
    ...
    regular_amount = forms.IntegerField(
        ...
        help_text=_(
            "We are happy for any donation. However, full membership with advantages, starts from CZK 150 per month."
        ),
        min_value=1,
    )
...
class PetitionUserForm_PetitionSignature(FieldNameMappingMixin, forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["event"].queryset = Event.objects.filter(
            slug__isnull=False, enable_signing_petitions=True
        ).exclude(slug="")
...
def get_or_create_new_user_profile(form):
    try:
        user = UserProfile.objects.get(
            profileemail__email=form.forms["userprofile"].cleaned_data["email"].lower()
        )
    ...
    if form.forms["userprofile"].cleaned_data["telephone"]:
        Telephone.objects.get_or_create(
            telephone=form.forms["userprofile"].cleaned_data["telephone"], user=user
        )
...
def update_or_create_new_payment_channel(form, new_user_profile):
    ...
    payment_channel, created = DonorPaymentChannel.objects.get_or_create(
        ...
        defaults={
            ...
            "expected_date_of_first_payment": timezone.now().date()
...
class RegularView(FormView):
    ...
    def success_page(
        ...
        response = render(
            ...
            {
                "amount": amount,
                "frequency": donor_frequency
                if donor_frequency != "---"
                else _("Onetime"),
                "account_number": bank_acc.first().bank_account_number
                if bank_acc
                else "",
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            email = self.get_post_param(
                request, "userprofile-email", "payment_data____email"
            )
            ...
            user_profiles = UserProfile.objects.filter(
                profileemail__email=email.lower()
            )
            if user_profiles.exists():
                payment_channels = user_profiles.get().userchannels.filter(
                    event__slug=event
                )
                if payment_channels.exists():
                    ...
                    autocom.check(
                        ...
                        action="resent-data-"
                        + self.request.POST["donorpaymentchannel-payment_type"],
                    )
                    user_data = {}
                    if "recurringfrequency" in request.POST:
                        user_data["frequency"] = REGULAR_FREQUENCY_MAP[
                            request.POST.get("recurringfrequency")
                        ]
                    else:
                        user_data["frequency"] = request.POST.get(
                            "donorpaymentchannel-regular_frequency"
                        )
                    user_data["name"] = self.get_post_param(
                        request, "userprofile-first_name", "payment_data____jmeno"
                    )
                    user_data["surname"] = self.get_post_param(
                        request, "userprofile-last_name", "payment_data____prijmeni"
                    )
                    user_data["amount"] = self.get_post_param(
                        request, "donorpaymentchannel-regular_amount", "amount"
                    )
                    user_data["telephone"] = self.get_post_param(
                        request, "userprofile-telephone", "payment_data____telefon"
                    )
    ...
    def form_valid(self, form):
        ...
        new_user_profile.administrative_units.add(
            payment_channel.money_account.administrative_unit
        )
        return self.success_page(
            payment_channel, form.clean()["donorpaymentchannel"]["money_account"].slug
        )
...
class DonatorsView(View):
    def get(self, request, unit):
        unit = get_object_or_404(AdministrativeUnit, slug=self.kwargs["unit"])
        users = Preference.objects.filter(
            administrative_unit=unit, public=True
        ).values_list("user")
...
def stat_members(request):
    members_by_months = (
        DonorPaymentChannel.objects.filter(user__is_active=True)
        ...
        .annotate(
            ...
            regular=Count(
                Case(
                    When(regular_payments="regular", then=1),
                    output_field=IntegerField(),
                )
            ),
            irregular=Count(
                Case(
                    When(~Q(regular_payments="regular"), then=1),
                    output_field=IntegerField(),
                )
            ),
        )
    ...
    return render(
        ...
        "stat-members.html",
        {
            ...
            "total_members": DonorPaymentChannel.objects.all()
...
def stat_payments(request):
    payments_by_months = (
        Payment.objects.filter(~Q(type="expected"))
    ...
    return render(
        ...
        "stat-payments.html",
        {
            ...
            "total_amount": Payment.objects.all()
...
class CampaignStatistics(View):
    ...
    def get(self, request, *args, **kwargs):
        campaign = get_object_or_404(
            Event, slug=kwargs["campaign_slug"], allow_statistics=True
        )
...
class PetitionSignatures(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        signatures = PetitionSignature.objects.filter(
            event=event, email_confirmed=True, public=True
        ).order_by("-created")
...
class SendMailingListView(SesameUserMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        preference.send_mailing_lists = (
            False if kwargs["unsubscribe"] == "unsubscribe" else True
        )
        preference.save()
        user_profiles = UserProfile.objects.filter(id=user.id)
        autocom.check(
            user_profiles=user_profiles, action="user-mailing-" + kwargs["unsubscribe"]
        )
