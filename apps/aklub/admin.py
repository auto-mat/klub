...
from flexible_filter_conditions.admin_filters import (
    UserConditionFilter,
...
from .forms import (
    ...
    CompanyProfileChangeForm,
    TaxConfirmationForm,
    UnitUserProfileAddForm,
    UnitUserProfileChangeForm,
    UserCreateForm,
    UserUpdateForm,
)
...
from .profile_model_resources import (
    ...
    get_polymorphic_parent_child_fields,
)
from .profile_model_resources_mixin import ProfileModelResourceMixin
from .utils import (
    annotation_friendly_search_results,
    check_annotate_filters,
    edit_donor_annotate_filter,
    sweet_text,
)
...
class DonorPaymentChannelInlineForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.id:
            self.fields[
                "user_bank_account_char"
            ].initial = self.instance.user_bank_account
        else:
            user_bank_accs = self.parent.userchannels
            if (
                user_bank_accs.exclude(user_bank_account=None)
                .values_list("user_bank_account")
                .distinct()
                .count()
                == 1
            ):
                ...
                val = (
                    bank_acc.user_bank_account.bank_account_number
                    if bank_acc.user_bank_account
                    else ""
                )
...
class DonorPaymentChannelInline(admin.StackedInline):
    ...
    def get_queryset(self, request):
        ...
        return (
            queryset.annotate(sum_amount=Sum("payment__amount"))
...
def show_payments_by_year(self, request, queryset):
    ...
    amount_string = [
        ...
        % (
            date_year.year,
            payments.filter(date__year=date_year.year).aggregate(Sum("amount"))[
                "amount__sum"
            ],
        )
...
class UserProfileLoaderClass(BaseInstanceLoader):
    def get_instance(self, row):
        ...
        if row.get("email"):
            try:
                obj = ProfileEmail.objects.get(
                    email=row["email"],
                    user__polymorphic_ctype__model=UserProfile._meta.model_name,
                ).user
...
class UserProfileResource(ProfileModelResourceMixin):
    class Meta:
        ...
        export_order = get_profile_admin_export_base_order_fields() + [
            "title_before",
            "first_name",
            "last_name",
            "title_after",
...
class CompanyProfileResource(ProfileModelResourceMixin):
    ...
    def dehydrate_telephone(self, profile):
        ...
        if contacts:
            return ",\n".join(
                contact.telephone if contact.telephone else "-" for contact in contacts
            )
        else:
            return "-"
    ...
    def dehydrate_email(self, profile):
        ...
        if contacts:
            return ",\n".join(
                contact.email if contact.email else "-" for contact in contacts
            )
        else:
            return "-"
    ...
    def dehydrate_contact_first_name(self, profile):
        ...
        if contacts:
            return ",\n".join(
                contact.contact_first_name if contact.contact_first_name else "-"
    ...
    def dehydrate_contact_last_name(self, profile):
        ...
        if contacts:
            return ",\n".join(
                contact.contact_last_name if contact.contact_last_name else "-"
                for contact in contacts
            )
        else:
            return "-"
...
class ProfileResource(ProfileModelResource):
    ...
    def before_import_row(self, row, **kwargs):
        ...
        row["polymorphic_ctype_id"] = ContentType.objects.get(
            model=row["profile_type"]
        ).id
    ...
    def import_field(self, field, obj, data, is_m2m=False):
        ...
        ):  # and not getattr(obj, field.column_name):
...
class ProfileEmailInline(admin.TabularInline):
    ...
    def is_email_in_companyprofile(self, obj):
        ...
        if not self.form.request.user.has_perm("can_edit_all_units"):
            ...
            ] = self.form.request.user.administrated_units.all()
            filter_kwargs[
...
class ApiAccountAdmin(
    ...
    def darujme_url(self, obj):
        if self.request.user.has_perm("aklub.can_edit_all_units"):
            is_correct = _boolean_icon(
                "pledges" in requests.get(obj.darujme_url()).json()
            )
            return mark_safe(
                f'<a href="{obj.darujme_url()}">{_(f"Here {is_correct}")}</a>'
            )
        else:
...
class ProfileAdminMixin:
        ...
        for channel in channels:
            if channel.regular_payments == "regular":
                ...
                if (
                    ...
                    and str(channel.event.id) not in self.filtered_events
                ):
                    continue
                ...
                # self.user_administrated_units is defined in queryset below
                    ...
                    channel.money_account.administrative_unit
                    in self.user_administrated_units
                ):
                    results.append(channel)
        return results
    ...
    def regular_amount(self, obj):
        result = self.get_donor_details(obj)
        return sweet_text(
            ((str(d.regular_amount),) for d in result if d.regular_amount)
        )
    ...
    def donor_delay(self, obj):
        def get_result(dpch):
            ...
            if delay is None:
                ...
                status = format_html(
                    "{} {} {}", mark_safe(_boolean_icon(False)), str(delay.days), "days"
                )
            return status
    ...
    def donor_extra_money(self, obj):
        result = self.get_donor_details(obj)
        return sweet_text(
            ((str(d.extra_money),) if d.extra_money else "-" for d in result)
        )
    ...
    def total_payment(self, obj):
        ...
        for unit in administrative_units:
            ...
            results.append(
                f"{unit}: {result['amount__sum']} Kč ({result['amount__count']})"
            )
        return ",\n".join(results)
    ...
    def change_view(self, request, object_id, extra_context=None, **kwargs):
        ...
        extra_context["urlsafe_query"] = query_to_base64(
            {
                "search_string": "OR".join(
                    [pe.email for pe in ProfileEmail.objects.filter(user__pk=object_id)]
                ),
                "search_profile_pks": [object_id],
            }
        )
        extra_context["display_fields"] = serializers.serialize(
        ...
        extra_context["required_fields"] = [
            field.name
            for field in Interaction._meta.get_fields()
    ...
    def taxform(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            return HttpResponseRedirect(
                reverse("admin:aklub_taxconfirmation_changelist")
            )
        else:
    ...
    def remove_contact_from_unit(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if profile == request.user:
                messages.warning(
                    request,
                    _("You can not remove administrative unit from your own profile"),
                )
            else:
                profile.administrative_units.remove(
                    request.user.administrated_units.first()
                )
                messages.info(
                    request,
                    _(
                        f'Profile with ID "{pk}" was removed from your administrative unit'
                    ),
                )
            ...
            if (
                request.user.administrated_units.first()
                not in profile.administrative_units.all()
            ):
                messages.warning(
                    request,
                    _(f"profile with ID '{pk}' doesn't exist. Perhaps it was deleted?"),
                )
    ...
    def get_instance(self, row):
        try:
            ...
            raise ValidationError(
                {"moneyaccount": _("MoneyAccount with this bank_number doesnt exist")}
            )
...
class DonorPaymentChannelResource(ModelResource):
    ...
    money_account = fields.Field(
        attribute="money_account", widget=DonorPaymentChannelWidget(MoneyAccount)
    )
    ...
    def import_obj(self, obj, data, dry_run):
        ...
        if data.get("user_bank_account"):
            user_bank_acc, _ = UserBankAccount.objects.get_or_create(
                bank_account_number=data.get("user_bank_account")
            )
    ...
    def dehydrate_email(self, donor):
        if hasattr(donor, "user"):
            if donor.user.polymorphic_ctype.model == UserProfile._meta.model_name:
                ...
                try:
                    email = donor.user.companyprofile.companycontact_set.get(
                        is_primary=True
                    )
...
class DonorPaymetChannelAdmin(
    ...
    def get_queryset(self, request):
        ...
        """
        primary_email_user = ProfileEmail.objects.filter(
            user=OuterRef("user"), is_primary=True
        )
...
add_user_bank_acc_to_dpch.short_description = _(
    "add user bank account  to current donor payment channel (rewrite)"
)
...
def payment_pair_action(self, request, queryset):
    ...
    messages.info(
        request, _("Payments succesfully paired with donor payment channels.")
    )


payment_pair_action.short_description = _(
    "pair payments from account statement with donor payment channels"
)
...
def payment_request_pair_action(self, request, queryset):
    if request.user.administrated_units.count() == 1:
        ...
        messages.info(
            request,
            _(
                "Payments succesfully paired with donor payment channels which are under your administrative unit."
            ),
        )
    else:
        messages.error(
            request, _("Your administrated unit have to be set to pair payments.")
        )
...
class PaymentResource(ModelResource):
    recipient_account = fields.Field(
        attribute="recipient_account", widget=PaymentWidget(MoneyAccount)
    )
    user_donor_payment_channel = fields.Field(
        attribute="user_donor_payment_channel",
    ...
    def get_queryset(self, request):
        ...
        if not request.user.has_perm("aklub.can_edit_all_units"):
            ...
            qs = qs.filter(
                ...
                | Q(
                    user_donor_payment_channel__money_account__administrative_unit=administrated_unit
                )
...
class MassCommunicationAdmin(
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .annotate(send_to_users_count=Count("send_to_users"))
        )
    ...
    def save_form(self, request, form, change):
        ...
        if "_continue" in request.POST and request.POST["_continue"] == "send_mails":
            try:
                ...
                messages.error(
                    request, _("While sending e-mails the problem occurred: %s") % e
                )
                raise e
...
pair_payment_with_dpch.short_description = _(
    "Pair payments with users based on variable symboles or user bank account"
)
...
class AccountStatementsAdmin(
    unit_admin_mixin_generator("administrative_unit"), nested_admin.NestedModelAdmin
):
    list_display = (
        "type",
        "import_date",
...
class TaxConfirmationAdmin(
    ...
    def get_queryset(self, request):
        ...
        """
        primary_email_user = ProfileEmail.objects.filter(
            user=OuterRef("user_profile_id"), is_primary=True
        )
        primary_email_company = CompanyContact.objects.filter(
            company=OuterRef("user_profile_id"),
            administrative_unit=OuterRef(
                "pdf_type__pdfsandwichtypeconnector__administrative_unit"
            ),
            is_primary=True,
        )
...
class UserProfileAdmin(
    ...
    advanced_filter_fields = (
        (
            ...
            "country",
            "zip_code",
            ("correspondence_street", _("Correspondence address street and number")),
            ("correspondence_city", _("Correspondence address city/city part")),
            ("correspondence_country", _("Correspondence address country")),
            ("correspondence_zip_code", _("Correspondence address ZIP code")),
    ...
    advanced_filter_fields_operators = {
        ...
        "title_before": ("iexact", "icontains", "isnull"),
        "first_name": ("iexact", "icontains"),
        "last_name": ("iexact", "icontains"),
        "title_after": ("iexact", "icontains", "isnull"),
        "sex": ("iexact", "isnull"),
        "is_staff": ("istrue", "isfalse"),
        "date_joined": ("range",),
        "last_login": ("range",),
        "age_group": ("lt", "iexact", "gt", "nirange", "isnull"),
        "birth_month": ("lt", "iexact", "gt", "nirange", "isnull"),
        "birth_day": ("lt", "iexact", "gt", "nirange", "isnull"),
        "street": ("iexact", "icontains"),
        "city": ("iexact", "icontains"),
        "country": ("iexact", "icontains"),
        "zip_code": ("iexact", "icontains"),
        "correspondence_street": ("iexact", "icontains"),
        "correspondence_city": ("iexact", "icontains"),
    ...
    def get_email(self, obj):
        ...
        if resolve(self.request.path_info).url_name == "aklub_userprofile_change":
            ...
            emails = (
                mark_safe(
                    '<a href="#profileemail_set-group">%(detail)s</a><br>'
                    % {"detail": _("Details")}
                )
                + emails
            )
        ...
        if data.get("email") and data.get("administrative_units"):
            try:
                ...
                unit = AdministrativeUnit.objects.get(
                    id=data.get("administrative_units")
                )
                user = email.user
                ...
                messages.warning(
                    request,
                    _(
                        "User With this email is in database already. You can edit him now"
                    ),
                )
    ...
    def get_queryset(self, request, *args, **kwargs):
        ...
        filter_kwargs = {}
        extra_related = []
        filter_kwargs = check_annotate_filters(
            self.list_display, request, filter_kwargs
        )
        # annotate_kwargs = check_annotate_subqueries(self, request)
        if not request.user.has_perm("aklub.can_edit_all_units"):
            donor_filter[
                "userchannels__money_account__administrative_unit__in"
            ] = self.user_administrated_units
        ...
        queryset = (
            ...
            .annotate(
                sum_amount=Sum(
                    "userchannels__payment__amount", filter=Q(**donor_filter)
                ),
                payment_count=Count("userchannels__payment", filter=Q(**donor_filter)),
                last_payment_date=Max(
                    "userchannels__payment__date", filter=Q(**donor_filter)
                ),
...
class CompanyContactInline(admin.TabularInline):
    ...
    def is_email_in_userprofile(self, obj):
        ...
        if not self.form.request.user.has_perm("can_edit_all_units"):
            filter_kwargs[
                "user__administrative_units__in"
            ] = self.form.request.user.administrated_units.all()
...
class CompanyProfileAdmin(
    child_redirect_mixin("companyprofile"),
    filters.AdministrativeUnitAdminMixin,
    ImportExportMixin,
    RelatedFieldAdmin,
    AdminAdvancedFiltersMixin,
    ...
    readonly_fields = BaseProfileChildAdmin.readonly_fields + (
        "get_company_email",
        "get_company_telephone",
    )
    advanced_filter_fields = (
    ...
    def get_company_email(self, obj):
        if self.request.user.has_perm("aklub.can_edit_all_units"):
            ...
            com = [
                c
                for c in obj.companycontact_set.all()
                if c.administrative_unit_id in self.user_administrated_units_ids
            ]
        ...
        if resolve(self.request.path_info).url_name == "aklub_companyprofile_change":
            emails = (
                mark_safe(
                    '<a href="#companycontact_set-group">%(detail)s</a><br>'
                    % {"detail": _("Details")}
                )
                + emails
            )
        ...
        if data.get("administrative_units"):
            try:
                ...
                messages.warning(
                    request,
                    _(
                        "Company is in database already. You are able to make changes now."
                    ),
                )
                url = reverse("admin:aklub_companyprofile_change", args=(company.pk,))
                return HttpResponseRedirect(url)
    ...
    def get_queryset(self, request, *args, **kwargs):
        # save request user's adminsitratived_unit here, so we dont have to peek in every loop
        self.user_administrated_units = request.user.administrated_units.all()
        self.user_administrated_units_ids = (
            request.user.administrated_units.all().values_list("id", flat=True)
        )
