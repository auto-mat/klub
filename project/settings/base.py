...
if AKLUB_ADMINS:
    ADMINS = [
        [s.strip() for s in admin.split(",")]
        for admin in AKLUB_ADMINS.strip().split("\n")
    ]
    MANAGERS = ADMINS
...
AKLUB_CORS_ORIGIN_REGEX_WHITELIST = os.environ.get(
    "AKLUB_CORS_ORIGIN_REGEX_WHITELIST", ""
).split(":")
...
SPAGHETTI_SAUCE = {
    "apps": [
        ...
        "pdf_storage",
        "aklub",
    ],  # include only main applications
    "show_fields": False,
